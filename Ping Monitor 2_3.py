...
def Stats(Address):
   ...
   def Find_Address():
      ...
      Tcount = 0
      TTime  = 0
      with open("PingMonitor.log") as file:
           for line in file:
                   Lines = (line.strip())
   ...
   label1 = tk.Label(cwin,text="Enter Address").pack()
...
def Save_Log(Oname,PassOrFail):
    ...
    file = open("PingMonitor.log","a")
    PingOutput = Oname+",\t\t"+PassOrFail+",\t"+ str(date.strftime("%d\%m\%y  %H:%M"))
